...
- [x] **Check SeasonalChecklist undocumented silent catch** — `src/components/portal/SeasonalChecklist.tsx:71` has a bare `} catch {}`. Investigate what's being silenced; add `console.error` at minimum.
- [x] **Check DigitalTwinTab silent catch** — `src/components/admin/DigitalTwinTab.tsx:589` has `catch { /* ignore */ }`. Verify intent and document or fix.
...
| 2026-04-17 (manual pickup) | SeasonalChecklist + DigitalTwinTab silent catches | — | Both now log via `console.error` with a comment explaining the fallback (malformed localStorage / edge-fn failure shouldn't crash the UI). Bundled in one PR. |
