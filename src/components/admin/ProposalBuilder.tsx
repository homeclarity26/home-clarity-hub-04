...
const THEME_SWATCHES = [
  { id: "navy", label: "Navy", hero: "#1a2744", accent: "#c9a96e" },
  { id: "slate", label: "Slate", hero: "#334155", accent: "#0ea5e9" },
  { id: "forest", label: "Forest", hero: "#1a3a2a", accent: "#84cc16" },
  { id: "midnight", label: "Midnight", hero: "#0f0f1a", accent: "#6366f1" },
  { id: "warm", label: "Warm", hero: "#7c3d1e", accent: "#e07b39" },
];
...
const ProposalBuilder = ({ estimate, lineItems, clientName, propertyAddress, onUpdate }: ProposalBuilderProps) => {
  ...
  const save = async () => {
    setSaving(true);
    await (supabase.from("estimates") as any).update(form).eq("id", estimate.id);
    setSaving(false);
      ...
      <Accordion type="multiple" defaultValue={["cover", "intro", "scope"]} className="space-y-2">
        ...
        <AccordionItem value="cover" className="border rounded-lg px-4">
          ...
          <AccordionContent className="space-y-3 pb-4">
            <div>
              <Label className="text-xs font-sans">Theme</Label>
              <div className="flex gap-2 mt-1.5">
                {THEME_SWATCHES.map((t) => (
                  <button
                    key={t.id}
                    onClick={() => update("proposal_color_theme", t.id)}
                    className={`w-10 h-10 rounded-lg border-2 transition-all ${form.proposal_color_theme === t.id ? "ring-2 ring-offset-2 ring-primary scale-110" : "border-transparent"}`}
                    style={{ background: `linear-gradient(135deg, ${t.hero} 60%, ${t.accent} 100%)` }}
                    title={t.label}
                  />
                ))}
              </div>
            ...
            <div>
              ...
              <Input value={form.proposal_cover_image_url} onChange={(e) => update("proposal_cover_image_url", e.target.value)} placeholder="https://..." className="text-xs" />
            </div>
            <div>
              <Label className="text-xs font-sans">Cover Video URL (optional)</Label>
              <Input value={form.proposal_cover_video_url} onChange={(e) => update("proposal_cover_video_url", e.target.value)} placeholder="YouTube/Vimeo URL" className="text-xs" />
            </div>
            <div>
              <Label className="text-xs font-sans">Prepared For</Label>
              <Input value={form.proposal_prepared_for} onChange={(e) => update("proposal_prepared_for", e.target.value)} className="text-xs" />
            </div>
            <div>
              <Label className="text-xs font-sans">Tagline</Label>
              <Input value={form.proposal_tagline} onChange={(e) => update("proposal_tagline", e.target.value)} placeholder="A complete clarity plan for your home" className="text-xs" />
            </div>
          </AccordionContent>
        </AccordionItem>
        ...
        <AccordionItem value="intro" className="border rounded-lg px-4">
          ...
          <AccordionContent className="space-y-3 pb-4">
            <Textarea value={form.proposal_intro_text} onChange={(e) => update("proposal_intro_text", e.target.value)} rows={5} className="text-xs" placeholder="Write a warm, personalized introduction..." />
            <Button size="sm" variant="outline" className="gap-1.5 text-xs" onClick={() => aiGenerate("proposal_intro_text", "proposal_intro")} disabled={!!aiLoading}>
              {aiLoading === "proposal_intro_text" ? <Loader2 className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3" />} Write with AI
            </Button>
          </AccordionContent>
        </AccordionItem>
            ...
            {(form.proposal_timeline_phases as any[]).map((phase: any, i: number) => (
              <Card key={i} className="p-3 space-y-2">
                <div className="flex gap-2">
                  <Input value={phase.icon || ""} onChange={(e) => {
                    const next = [...form.proposal_timeline_phases as any[]];
                    next[i] = { ...next[i], icon: e.target.value };
                    update("proposal_timeline_phases", next);
                  }} className="w-14 text-center text-xs" placeholder="📋" />
                  <Input value={phase.phase_title || ""} onChange={(e) => {
                    const next = [...form.proposal_timeline_phases as any[]];
                    next[i] = { ...next[i], phase_title: e.target.value };
                    update("proposal_timeline_phases", next);
                  }} className="text-xs flex-1" placeholder="Phase title" />
                  <Input value={phase.duration || ""} onChange={(e) => {
                    const next = [...form.proposal_timeline_phases as any[]];
                    next[i] = { ...next[i], duration: e.target.value };
                    update("proposal_timeline_phases", next);
                  }} className="text-xs w-24" placeholder="Week 1-2" />
                  <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={() => {
                    update("proposal_timeline_phases", (form.proposal_timeline_phases as any[]).filter((_: any, idx: number) => idx !== i));
                  }}><Trash2 className="w-3 h-3" /></Button>
                </div>
                <Textarea value={phase.description || ""} onChange={(e) => {
                  ...
                  next[i] = { ...next[i], description: e.target.value };
                  update("proposal_timeline_phases", next);
                }} className="text-xs" rows={2} placeholder="Description..." />
              </Card>
            ))}
            <div className="flex gap-2">
              <Button variant="outline" size="sm" className="text-xs gap-1" onClick={() => {
                update("proposal_timeline_phases", [...(form.proposal_timeline_phases as any[]), { icon: "📋", phase_title: "", duration: "", description: "" }]);
              }}><Plus className="w-3 h-3" /> Add Phase</Button>
              <Button variant="outline" size="sm" className="text-xs gap-1" onClick={() => aiGenerate("proposal_timeline_phases", "proposal_timeline")} disabled={!!aiLoading}>
                {aiLoading === "proposal_timeline_phases" ? <Loader2 className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3" />} Generate Timeline
              </Button>
            </div>
          </AccordionContent>
        </AccordionItem>
        ...
        <AccordionItem value="why-us" className="border rounded-lg px-4">
          ...
          <AccordionContent className="space-y-3 pb-4">
            <Textarea value={form.proposal_why_us_text} onChange={(e) => update("proposal_why_us_text", e.target.value)} rows={4} className="text-xs" placeholder="Why clients should choose you..." />
            <Label className="text-xs font-sans">Stat Callouts</Label>
            {(form.proposal_stat_callouts as any[]).map((stat: any, i: number) => (
              <div key={i} className="flex gap-2 items-center">
                <Input value={stat.icon} onChange={(e) => { const n = [...form.proposal_stat_callouts as any[]]; n[i] = { ...n[i], icon: e.target.value }; update("proposal_stat_callouts", n); }} className="w-12 text-center text-xs" placeholder="🏠" />
                <Input value={stat.number} onChange={(e) => { const n = [...form.proposal_stat_callouts as any[]]; n[i] = { ...n[i], number: e.target.value }; update("proposal_stat_callouts", n); }} className="w-20 text-xs" placeholder="200+" />
                <Input value={stat.label} onChange={(e) => { const n = [...form.proposal_stat_callouts as any[]]; n[i] = { ...n[i], label: e.target.value }; update("proposal_stat_callouts", n); }} className="flex-1 text-xs" placeholder="homes managed" />
                <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={() => update("proposal_stat_callouts", (form.proposal_stat_callouts as any[]).filter((_: any, idx: number) => idx !== i))}><Trash2 className="w-3 h-3" /></Button>
              </div>
            ))}
            <Button variant="outline" size="sm" className="text-xs gap-1" onClick={() => update("proposal_stat_callouts", [...(form.proposal_stat_callouts as any[]), { icon: "⭐", number: "", label: "" }])}><Plus className="w-3 h-3" /> Add Stat</Button>
          </AccordionContent>
        </AccordionItem>
        ...
        <AccordionItem value="testimonial" className="border rounded-lg px-4">
          ...
          <AccordionContent className="space-y-3 pb-4">
            <Textarea value={form.proposal_testimonial_quote} onChange={(e) => update("proposal_testimonial_quote", e.target.value)} rows={3} className="text-xs" placeholder="Client testimonial quote..." />
        ...
        <AccordionItem value="cta" className="border rounded-lg px-4">
          ...
          <AccordionContent className="space-y-3 pb-4">
            <div><Label className="text-xs font-sans">CTA Headline</Label><Input value={form.proposal_cta_headline} onChange={(e) => update("proposal_cta_headline", e.target.value)} className="text-xs" /></div>
            <div><Label className="text-xs font-sans">CTA Subtext</Label><Textarea value={form.proposal_cta_subtext} onChange={(e) => update("proposal_cta_subtext", e.target.value)} rows={2} className="text-xs" /></div>
          </AccordionContent>
        </AccordionItem>
      </Accordion>
      ...
      {(estimate.proposal_view_count || 0) > 0 && (
        <Card className="p-4 space-y-2">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div><p className="text-[10px] text-muted-foreground font-mono uppercase">Views</p><p className="text-lg font-bold">{estimate.proposal_view_count || 0}</p></div>
            <div><p className="text-[10px] text-muted-foreground font-mono uppercase">Time Spent</p><p className="text-lg font-bold">{Math.round((estimate.proposal_time_spent_seconds || 0) / 60)}m</p></div>
            <div><p className="text-[10px] text-muted-foreground font-mono uppercase">Sections Viewed</p><p className="text-lg font-bold">{(estimate.proposal_sections_viewed || []).length}</p></div>
          </div>
          {estimate.proposal_first_viewed_at && (
            <p className="text-[10px] text-muted-foreground">First viewed: {new Date(estimate.proposal_first_viewed_at).toLocaleDateString()}</p>
          )}
        </Card>
      )}
    </div>
