...
| 10–11 | Step 3 Furnace | IDENTIFICATION / LIFECYCLE groups, condition segmented control, PHOTOS (REQUIRED) w/ REQUIRED pill, lifecycle timeline + alert in preview | ◐ 2026-07-07 Phase 5b diff vs proto_10 (`?scenario=wizard-step3-system`, qa_wizard_step3_system.png). IDENTIFICATION (make/model/serial/install) + LIFECYCLE (lifespan, computed Estimated End-of-Life "2029-2031", condition control) built; preview renders the real SystemTemplatePage (spec grid, gradient lifecycle timeline, proactive alert). PHOTOS (REQUIRED) group still pending (photo intake lives in Step 5) |
